...
- **任何動到公開函式簽章的變更**——`list_agents`、`read_agent`、`send_to_agent`、
  ...
  為什麼要改。這些簽章之所以在 `0.1.0` 就凍結,是刻意讓未來可能建立在這個套件之上的政策層可以放心依賴它們;
...
`mode` 語意的邏輯(超出單純記錄的範圍)。那類邏輯屬於一個獨立於這個 repo 之外、建立在這個
套件之上的政策/排程層,透過公開的 `BridgeActions` 介面來使用這個套件——完整介面約定見
[`docs/api.md`](docs/api.md)。

具體來說:**在這個 repo 加政策或排程邏輯的 PR,會被引導到別的地方去,不會被合併。** 如果
