...
1. **方案 B:additive 新模組 `herdr_bridge.acp`**(0.2.0 minor)。六個 frozen API 一字不動;新契約(AcpActions:ensure_session/prompt/start_prompt+wait_done/cancel)並存。否決「雙軌 backend 塞舊 API」(pane-centric vs session-centric 本體論不合)與「治理層直調 acpx」(alpha 直穿政策層+audit 斷裂)。
...
6. **遷移**:M0 spike(12+ 驗證項,四 kill criteria:model 指定/版本協商/可視性/並行污染)→ M1 工具層模組(provisional tier)→ M2 治理層雙軌(fleet.yaml per-tier dispatch_backend,≥2 週或 ≥20 件實戰)→ M3 權限映射。每階段可回退;M2 回退=設定翻轉。
7. **權限**:起步 --approve-all(=現況等權,audit 標記無 agent 間隔離)→ S1 白名單 → S2 deny-default;M2 補 worktree 隔離 opt-in→opt-out 觸發條件 ADR。

## Consequences

- 治理層 dispatch 從 marker predicate 改 branch on PromptResult.reason/stop_reason;named session 讓 agent_id 跨重啟失效之痛消失。
...
- **範圍調整**：只接好 `agent="opencode"` 這個 tier（§4.5 規劃的具名 acpx agent 條目多 tier 支援待治理層設定後擴充，非本輪範圍）；`cancel()` 是直接終止子行程而非協定層 `session/cancel`；`prompt()` 的 `policy` 參數保留簽名形狀但不生效（policy 只在 `ensure_session()` 決定一次）。
...
- **M2/M3**（治理層雙軌切換、權限映射正式導入）維持未開工，待對應的批准關卡。
