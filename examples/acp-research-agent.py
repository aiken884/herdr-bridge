...
"""
Simple ACP research agent for testing router / cross agent comms (distinct real downstream).
Uses agent-client-protocol SDK. Returns different response text for registry/choose testing.

Run as: uv run python examples/acp-research-agent.py
Register as "research-tui" in AcpRouter for capability-based routing demo.
"""
...
class ResearchAgent(Agent):
    """Minimal research-style ACP agent. Distinct from echo for real downstream tests.
    ...
    async def new_session(self, cwd: str, **kwargs):
        return {"session_id": "research-sess"}
    ...
    async def prompt(self, session_id: str, prompt: list, **kwargs) -> PromptResponse:
        ...
        # distinct "research" behavior
        response_text = f"Research result from ACP downstream agent: {user_text[:200]} [research ack via ACP]"

        print(f"[research-agent] received research query: {user_text[:100]}...", file=sys.stderr)
        ...
        try:
            ...
            print(f"[research-agent] sent side-channel report for {tid}", file=sys.stderr)
        except OSError as e:
            print(f"[research-agent] report send err (non-fatal): {e}", file=sys.stderr)

        # return with _meta; router collects "echo_text" or "result_text" for generality
    ...
    # other methods stub for full ACP compat
...
async def main():
    agent = ResearchAgent()
    print("Starting ACP Research Agent (distinct real downstream for herdr-bridge router demo)...", file=sys.stderr)
